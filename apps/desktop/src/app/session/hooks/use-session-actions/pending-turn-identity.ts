...
import { conflictingTranscriptIdentity, transcriptOccurrenceIds } from '@/lib/chat-messages/occurrence-identity'
...
export function persistedTurnsEquivalent(a: ChatMessage['persistedTurn'], b: ChatMessage['persistedTurn']): boolean {
  return (
    ...
    Boolean(
      ...
      a.user_display_order === b.user_display_order &&
      a.final_assistant_row_id === b.final_assistant_row_id &&
      a.final_assistant_display_order === b.final_assistant_display_order &&
...
export function acknowledgedTranscriptBoundary(next: ChatMessage[], previous: ChatMessage[]) {
  ...
  for (let localIndex = previous.length - 1; localIndex >= 0; localIndex -= 1) {
    ...
    const finalRowId =
      local.persistedTurn?.final_assistant_display_order ??
      local.persistedTurn?.final_assistant_row_id ??
      local.displayOrder ??
      local.rowId
