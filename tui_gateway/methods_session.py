...
def _(rid, params: dict) -> dict:
    ...
    messages = _history_to_messages(history, profile_home=profile_home or _hermes_home)
    return _ok(rid, {
...
class _Resume:
    ...
    def messages(self, display: list) -> list:
        return [] if self.omit_messages else _history_to_messages(
            display, profile_home=self.profile_home or _hermes_home)
...
def _(rid, params: dict, session: dict) -> dict:
    ...
    return _ok(rid, {"count": len(history), "messages": _history_to_messages(
        history, profile_home=session.get("profile_home") or _hermes_home)})
...
def _compress_via_compute_host(rid, params: dict, session: dict) -> dict:
    ...
    return _ok(rid, {
        ...
        "messages": _history_to_messages(ack.get("messages"), profile_home=session.get("profile_home") or _hermes_home)
        if isinstance(ack.get("messages"), list) else [],
...
def _compress_live(rid, sid: str, session: dict, focus_topic: str) -> dict:
    ...
    try:
        ...
        return _ok(rid, {
            ...
            "usage": usage, "info": info, "messages": _history_to_messages(
                messages, profile_home=session.get("profile_home") or _hermes_home)})
    finally:
...
def _(rid, params: dict, session: dict) -> dict:
    ...
    return _ok(rid, {"session_id": new_sid, "stored_session_id": new_key, "title": title, "parent": old_key,
                     "message_count": len(history), "messages": _history_to_messages(
                         history, profile_home=session.get("profile_home") or _hermes_home),
