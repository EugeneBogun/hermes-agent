...
def _format_live_history_output(sid: str, session: dict, arg: str) -> str:
    ...
    messages = _history_to_messages(
        history if db_history is None else db_history, profile_home=session.get("profile_home") or _hermes_home)
    if not messages:
...
def _format_live_context_output(sid: str, session: dict, arg: str) -> str:
    ...
    try:
        messages = _history_to_messages(
            _live_session_messages(session) or [], profile_home=session.get("profile_home") or _hermes_home)
    except Exception:
        messages = []  # malformed db rows fall back to the live history below
    if not messages:
        with session["history_lock"]:
            messages = _history_to_messages(
                list(session.get("history", [])), profile_home=session.get("profile_home") or _hermes_home)
    usage = _session_usage_snapshot(session)
    mirror = _metadata_mirror(session)
